...
async def test_queue_detail_structure_regression(mocker):
    ...
    async with app.run_test():
        ...
        # Children expected: 0:Header Label, 1:Count Label, 2:Vertical List
        ...
        # 2. Check technical names exist in the Vertical area
        source_list = detail.query_one("#source_props_list", Vertical)
        prop_names = [getattr(c, "tech_name") for c in source_list.children if hasattr(c, "tech_name")]
        assert "queries" in prop_names
        
        dest_list = detail.query_one("#dest_props_list", Vertical)
