import pytest
from typer.testing import CliRunner
from unittest.mock import patch

runner = CliRunner()

@pytest.fixture(scope="function")
def setup_test_campaign(tmp_path):
    campaigns_dir = tmp_path / "campaigns"
    campaigns_dir.mkdir()

    campaign_name = "test-campaign"
    campaign_dir = campaigns_dir / campaign_name
    campaign_dir.mkdir()

    config_content = "[campaign]\nname = \"Test Campaign\"\n"
    (campaign_dir / "config.toml").write_text(config_content)

    readme_content = "# Test Campaign\n"
    (campaign_dir / "README.md").write_text(readme_content)

    with patch('cocli.core.config.paths.root', tmp_path):
        yield campaign_name, campaign_dir

def test_campaign_edit_with_editor_configured(cli_app, setup_test_campaign):
    campaign_name, campaign_dir = setup_test_campaign
    config_path = campaign_dir / "config.toml"
    readme_path = campaign_dir / "README.md"
    editor_cmd = "my-editor"

    with patch('cocli.commands.campaign.mgmt.get_editor_command', return_value=editor_cmd) as mock_get_editor_command, \
         patch('subprocess.run') as mock_subprocess_run:
        result = runner.invoke(cli_app, ["campaign", "edit", campaign_name])

        assert result.exit_code == 0
        mock_get_editor_command.assert_called_once()
        mock_subprocess_run.assert_called_once_with([editor_cmd, str(config_path), str(readme_path)])

def test_campaign_edit_with_vim_editor(cli_app, setup_test_campaign):
    campaign_name, campaign_dir = setup_test_campaign
    config_path = campaign_dir / "config.toml"
    readme_path = campaign_dir / "README.md"
    editor_cmd = "nvim"
    with patch('cocli.commands.campaign.mgmt.get_editor_command', return_value=editor_cmd) as mock_get_editor_command, \
         patch('subprocess.run') as mock_subprocess_run:
        result = runner.invoke(cli_app, ["campaign", "edit", campaign_name])

        assert result.exit_code == 0
        mock_get_editor_command.assert_called_once()
        mock_subprocess_run.assert_called_once_with([editor_cmd, "-o", str(config_path), str(readme_path)])

def test_campaign_edit_no_editor_configured(cli_app, setup_test_campaign):
    campaign_name, campaign_dir = setup_test_campaign
    config_path = campaign_dir / "config.toml"

    with patch('cocli.commands.campaign.mgmt.get_editor_command', return_value=None) as mock_get_editor_command, \
         patch('typer.edit') as mock_typer_edit:
        result = runner.invoke(cli_app, ["campaign", "edit", campaign_name])

        assert result.exit_code == 0
        mock_get_editor_command.assert_called_once()
        mock_typer_edit.assert_called_once_with(filename=str(config_path))
        import re
        def strip_ansi(text):
            ansi_escape = re.compile(r'\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])')
            return ansi_escape.sub('', text)
        clean_output = strip_ansi(result.stdout)
        normalized_output = re.sub(r'\s+', ' ', clean_output).strip()
        assert "To edit the README.md as well, please configure an editor" in normalized_output

def test_campaign_edit_with_fzf(cli_app, setup_test_campaign):
    campaign_name, campaign_dir = setup_test_campaign
    config_path = campaign_dir / "config.toml"
    readme_path = campaign_dir / "README.md"
    editor_cmd = "my-editor"

    with patch('cocli.commands.campaign.mgmt.get_editor_command', return_value=editor_cmd), \
         patch('cocli.commands.campaign.mgmt.run_fzf', return_value=campaign_name) as mock_fzf, \
         patch('subprocess.run') as mock_subprocess_run:
        result = runner.invoke(cli_app, ["campaign", "edit"])

        assert result.exit_code == 0
        mock_fzf.assert_called_once()
        mock_subprocess_run.assert_called_once_with([editor_cmd, str(config_path), str(readme_path)])

def test_campaign_edit_no_campaign_selected(cli_app, setup_test_campaign):
    with patch('cocli.commands.campaign.mgmt.run_fzf', return_value=None):
        result = runner.invoke(cli_app, ["campaign", "edit"])

        assert result.exit_code == 1
        assert "No campaign selected" in result.stdout

def test_campaign_edit_no_campaigns_exist(cli_app, tmp_path):
    with patch('cocli.core.config.paths.root', tmp_path):
        result = runner.invoke(cli_app, ["campaign", "edit"])

        assert result.exit_code == 1
        assert "No campaigns found" in result.stdout

def test_campaign_edit_campaign_not_found(cli_app, tmp_path):
    # Isolate paths so leftover dirs under the shared COCLI_DATA_HOME cannot
    # make a "non-existent" campaign look real.
    with patch("cocli.core.config.paths.root", tmp_path):
        result = runner.invoke(cli_app, ["campaign", "edit", "non-existent-campaign"])

    assert result.exit_code == 1
    assert "not found" in result.stdout