...
def cleanup_ledger():
    """Clean up global ledger before and after each test."""
...
def test_usv_schema_append_only(tmp_path: Path):
    ...
    # 2. Save V2 (Good)
    MockModelV1.save_datapackage(
        tmp_path, "test", "*.usv"
    )  # Re-save same should be fine
    MockModelV2_Good.save_datapackage(
        tmp_path, "test", "*.usv"
    )  # Append should be fine
...
def test_usv_schema_blocks_reorder(tmp_path: Path):
    ...
    assert "Schema hash mismatch" in str(excinfo.value)
    assert "Old fields" in str(excinfo.value)
    assert "New fields" in str(excinfo.value)
