...
@pytest.mark.asyncio
async def test_scrape_google_maps_returns_data():
    """
    ...
    """
    with patch('cocli.scrapers.google_maps.get_coordinates_from_zip') as mock_get_coordinates, \
         patch('cocli.scrapers.google_maps.GoogleMapsCache') as mock_cache, \
         patch('cocli.scrapers.google_maps.ScrapeIndex') as mock_scrape_index, \
        ...
        mock_scrape_index_instance = mock_scrape_index.return_value
        mock_scrape_index_instance.is_area_scraped.return_value = None

        mock_cache_instance = mock_cache.return_value
        mock_cache_instance.get_by_place_id.return_value = None
        ...
        # Fix: The 'all' method should be on an awaitable object
        ...
        results = []
        async for result in scrape_google_maps(
        ...
        assert isinstance(results[0], GoogleMapsData)
...
def test_extract_business_data_basic():
    ...
    data = parse_business_listing_html(SAMPLE_ITEM_HTML, keyword, debug=True)
