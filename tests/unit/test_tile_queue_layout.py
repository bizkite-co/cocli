"""map-tile StationDecl — pure tile registry (pending/completed only, no
processing phase - removed 2026-08-09), tiles as layout under pending."""
...
def test_map_tile_station_phases_and_no_shard() -> None:
    ...
    assert not hasattr(ph, "processing") or getattr(ph, "processing", None) is None
...
def test_tile_queue_phase_dirs_and_tiles_layout(tmp_path: Path) -> None:
    with patch("cocli.core.paths.paths.root", tmp_path):
        ...
        assert not hasattr(q, "processing_dir")
...
def test_tile_ack_moves_pending_to_completed_and_uses_layout_s3_key(tmp_path: Path) -> None:
    mock_s3 = MagicMock()
    with patch("cocli.core.paths.paths.root", tmp_path):
        q = FilesystemTileQueue(
            "camp", s3_client=mock_s3, bucket_name="b"
        )
        tile = q.pending_dir / "t1.usv"
        tile.write_text("row\n")
