...
def mock_to_call_env(tmp_path, monkeypatch):
    ...
    task = ToCallTask(
        company_slug=slug, domain="test1.com", campaign_name=campaign, ack_token=None
    )
    task.save()
    ...
    assert str(data_home.resolve()) in str(task_path.resolve()), (
        f"LEAKAGE DETECTED: Task path {task_path} is not in {data_home}"
    )
...
def test_to_call_duckdb_integration(mock_to_call_env):
    ...
    get_fuzzy_search_results(
        "", item_type="company", campaign_name=campaign, force_rebuild_cache=True
    )
    ...
    assert any(r[0] == slug for r in cache_rows), (
        f"Slug '{slug}' must exist in items_cache"
    )
    ...
    assert to_call_rows[0][0] == slug, (
        f"Slug in table '{to_call_rows[0][0]}' should match '{slug}'"
    )

    # Step 4: Check JOIN view (items)
    view_rows = _con.execute(
        "SELECT slug, is_to_call FROM items WHERE slug = ?", [slug]
    ).fetchone()
    ...
    assert counts.get("tpl_to_call") == 1, (
        f"Template count for 'tpl_to_call' should be 1, got {counts.get('tpl_to_call')}"
    )
...
def test_to_call_removal_sync(mock_to_call_env):
    ...
    assert len(results) == 0, (
        "Company should disappear from To-Call results after file deletion"
    )
