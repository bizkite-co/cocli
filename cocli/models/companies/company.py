    ...
    def from_directory(cls, company_dir: Path) -> Optional["Company"]:
        ...
        try:
            ...
            if content.startswith("---") and "---" in content[3:]:
                ...
                if len(parts) >= 3:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping {company_dir.name}: YAML error in _index.md: {e}"
                        )
                        return None
            ...
            if tags_path.exists():
                tags = [
                    t.strip()
                    for t in tags_path.read_text().strip().split("\n")
                    if t.strip()
                ]
            ...
            if "email" in frontmatter_data and frontmatter_data["email"]:
                ...
                if (
                    not is_valid_email(email_val)
                    or email_val.startswith("[")
                    or email_val == "None"
                    or email_val == "null"
                ):
                    frontmatter_data["email"] = None

            if "all_emails" in frontmatter_data and isinstance(
                frontmatter_data["all_emails"], list
            ):
                cleaned_emails = []
                for email_val in frontmatter_data["all_emails"]:
                    if isinstance(email_val, str):
                        ...
                        isinstance(email_val, list)
                        and len(email_val) > 0
                        and isinstance(email_val[0], str)
                    ):
            ...
            try:
                ...
                logger.debug(
                    f"Skipping {company_dir.name}: Validation error loading company: {e}"
                )
                return None
            except Exception as e:
                logger.debug(
                    f"Skipping {company_dir.name}: Unexpected error loading company: {e}"
                )
                return None
        except Exception as e:
    ...
    def merge_with(self, other: "Company") -> None:
        ...
        for field in [
            ...
            if new_val is not None and (
                current_val is None or current_val == "" or current_val == "N/A"
            ):
