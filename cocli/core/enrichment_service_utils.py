...
def ensure_enrichment_service_ready(console: Console) -> None:
    ...
    """
    service_url = get_enrichment_service_url()
    is_remote = "localhost" not in service_url and "127.0.0.1" not in service_url

    if not is_remote:
        # --- Check for running container (Local Only) ---
        ...
        if result.returncode == 1: # No running container found
         ...
         console.print(f"[grey50][{datetime.now().strftime('%H:%M:%S')}][/] [dim]Using remote enrichment service at {service_url}...[/dim]")
    ...
    try:
        ...
        console.print(f"[bold red]Error: Could not connect to the enrichment service at {service_url}.[/bold red]")
        if not is_remote:
             console.print(
