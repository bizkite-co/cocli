...
class ReportingService:
    ...
    def generate_coverage_kml(
        ...
        """
        Generate per-phrase and aggregated-grid coverage KML files.
        ...
        if not scraped_areas:
            return VizExportResult(
                campaign_name=name,
                success=True,
                message="No scraped areas found.",
        ...
        phrases = sorted({area.phrase for area in scraped_areas})
        ...
        for phrase, areas in areas_by_phrase.items():
            ...
            kml_content = (
                ...
                f"<Document>{kml_body}</Document></kml>"
            )
        ...
        for tile_id, data in aggregated_tiles.items():
            ...
            coordinates = (
                f"{lon_min},{lat_min},0 {lon_max},{lat_min},0 "
            ...
            sorted_phrases = sorted(
                data["phrases"].items(), key=lambda x: x[1], reverse=True
            )
            for phrase, count in sorted_phrases:
                desc_parts.append(
                    f"<tr><td>{phrase}</td><td align='right'>{count}</td></tr>"
                )
            desc_parts.append(
                f"<tr><td><b>Total</b></td>"
                f"<td align='right'><b>{data['total_items']}</b></td></tr>"
            )
        ...
        return VizExportResult(
            campaign_name=name,
            success=True,
            message=f"Coverage visualization complete. Files saved in: {export_dir}",
            export_dir=export_dir,
            files=written,
            count=len(scraped_areas),
        )
    ...
    def generate_publish_kmls(
        ...
        steps = [
            ...
            ["campaign", "visualize-legacy-scrapes", name],
    ...
    def upload_kml_layers(
        ...
        files_to_upload = [
            ...
            (campaign_dir / "exports" / "target-areas.kml", f"kml/{name}_targets.kml"),
            (
                campaign_dir / "exports" / "legacy_scrapes.kml",
                f"kml/{name}_legacy.kml",
            ),
            (campaign_dir / f"{name}_prospects.kml", f"kml/{name}_prospects.kml"),
            (campaign_dir / f"{name}_customers.kml", f"kml/{name}_customers.kml"),
        ]
        ...
        layers = [
            ...
            {
                "name": "Scraped Areas",
                "url": f"https://{domain}/kml/{name}_aggregated.kml",
                "default": True,
            },
            {
                "name": "Legacy Scrapes",
                "url": f"https://{domain}/kml/{name}_legacy.kml",
                "default": False,
            },
            {
                "name": "Prospects",
