...
_last_filter_in_mtime: float = -1.0
_last_filter_out_mtime: float = -1.0
            ...
            try:
                ...
                row = _con.execute(f"""
                    SELECT
                      ...
                      COUNT(*) FILTER (WHERE type = 'company' AND is_filter_in = TRUE),
                      COUNT(*) FILTER (WHERE type = 'company' AND is_filter_out = TRUE)
                    FROM items
                """).fetchone()
                if row:
                    ...
                    counts["tpl_filter_in"] = int(row[10] or 0)
                    counts["tpl_filter_out"] = int(row[11] or 0)
...
def get_fuzzy_search_results(
    ...
    global \
        ...
        _last_email_mtime, \
        _last_filter_in_mtime, \
        _last_filter_out_mtime, \
        _last_campaign
    ...
    if campaign:
        ...
        filter_in_path = lead_filter_dir / "in.usv"
        filter_out_path = lead_filter_dir / "out.usv"
    ...
    with _lock:
        ...
        current_filter_in_mtime = (
            os.path.getmtime(filter_in_path)
            if filter_in_path and filter_in_path.exists()
            else -1.0
        )
        current_filter_out_mtime = (
            os.path.getmtime(filter_out_path)
        ...
        try:
            if _con is None:
                ...
                _last_filter_in_mtime = -1.0
                _last_filter_out_mtime = -1.0
            ...
            if (
                ...
                or _last_filter_in_mtime != current_filter_in_mtime
                or _last_filter_out_mtime != current_filter_out_mtime
            ):
                ...
                # Lead-filter membership: static slug lists regenerated by a
                # campaign's filter script (e.g. cocli/data/campaigns/<c>/
                # indexes/lead-filter/{in,out}.usv), always "current" state
                # - not versioned tags on the company record, and not an
                ...
                _con.execute(f"""
                    ...
                    LEFT JOIN items_filter_in lfi ON COALESCE(t1.slug, t2.slug) = lfi.slug
                    LEFT JOIN items_filter_out lfo ON COALESCE(t1.slug, t2.slug) = lfo.slug
                ...
                _last_filter_in_mtime = current_filter_in_mtime
                _last_filter_out_mtime = current_filter_out_mtime
            ...
            if filters:
                ...
                if filters.get("filter_in"):
                    sql += " AND is_filter_in = TRUE"
                if filters.get("filter_out"):
                    sql += " AND is_filter_out = TRUE"
