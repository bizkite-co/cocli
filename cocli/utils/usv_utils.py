...
class USVReader:
    ...
    def __init__(self, f: Any):
        ...
        if isinstance(self.content, str):
            content_bytes = self.content.encode("utf-8", errors="replace")
        else:
            content_bytes = self.content
        normalized = normalize_usv_record_separators(content_bytes)
...
def normalize_usv_record_separators(content: bytes) -> str:
    """Normalize USV record separators for parsing.

    Handles legacy files that may contain RS (\\x1e) as record separator.
    Step 1: \\x1e\\n → \\n  (RS followed by LF)
    Step 2: \\x1e → \\n    (RS alone becomes LF)
    Step 3: \\n\\n → \\n   (collapse consecutive LFs)

    This ensures compatibility between old files using RS and modern tools
    that expect LF as record separator.
    """
