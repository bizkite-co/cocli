        ...
        if char == "f":
            ...
            if email:
                ...
                try:
                    ...
                    console.print(
                        f"[bold green]Email '{email}' added. Press any key to continue.[/bold green]"
                    )
                except Exception as e:
                    console.print(
                        f"[bold red]Error adding email: {e}. Press any key to continue.[/bold red]"
                    )
            else:
                console.print(
                    "[bold yellow]No email entered. Press any key to continue.[/bold yellow]"
                )
            _getch()
        elif char == "X":
            ...
            if not campaign:
                console.print(
                    "[bold red]No active campaign set. Use 'cocli campaign set <campaign_name>' to set one. Press any key to continue.[/bold red]"
                )
                _getch()
                continue
            ...
            if reloaded_company is None:
                console.print(
                    "[bold red]Error: Could not reload company data after exclusion. Press any key to continue.[/bold red]"
                )
                _getch()
                continue
            ...
            if company.domain:  # Now company is guaranteed not to be None
                ...
                console.print(
                    f"[bold red]Company {company.name} excluded from campaign '{campaign}'. Press any key to continue.[/bold red]"
                )
        elif char == "a":
            console.print("\n[bold green]Adding a new meeting...[/bold green]")
            meeting_date_str = typer.prompt(
                "Enter meeting date (e.g., 'today', 'next Monday', '2025-12-25')"
            )
            _add_meeting_logic(company_name=company_slug, date_str=meeting_date_str)
            console.print(
                "[bold green]Meeting added. Press any key to continue.[/bold green]"
            )
            ...
            console.print(
                "[bold green]_index.md closed. Press any key to continue.[/bold green]"
            )
            _getch()  # Wait for a key press to clear the message
        elif char == "w":
            ...
            if domain:
                ...
                try:
                    if open_url(url):
                        console.print(
                            f"[bold green]Opened {url} in browser. Press any key to continue.[/bold green]"
                        )
                    else:
                        console.print(
                            f"[bold red]Could not open browser for {url}[/bold red]"
                        )
                except Exception as e:
                    console.print(f"[bold red]Error opening browser: {e}[/bold red]")
            else:
                console.print(
                    "[bold red]No domain found for this company. Press any key to continue.[/bold red]"
                )
            _getch()  # Wait for a key press to clear the message
        elif char == "p":
            ...
            if phone_number:
                ...
                try:
                    if provider.dial(phone_number, campaign):
                        console.print(
                            f"[bold green]Initiated call to {phone_number}. Auto-creating meeting...[/bold green]"
                        )
                    else:
                        console.print(
                            f"[bold red]Could not open browser to call {phone_number}[/bold red]"
                        )
                    _add_meeting_logic(
                        company_name=company_slug,
                        date_str="today",
                        title_str="Google Voice Call",
                        phone_number_str=phone_number,
                    )
                    console.print(
                        "[bold green]Meeting for call added. Press any key to continue.[/bold green]"
                    )
                except Exception as e:
                    console.print(
                        f"[bold red]Error initiating call: {e}. Press any key to continue.[/bold red]"
                    )
            else:
                console.print(
                    "[bold red]No phone number found for this company. Press any key to continue.[/bold red]"
                )
            _getch()  # Wait for a key press to clear the message
        elif char == "m":
            ...
            if meetings_data:
                ...
                meeting_map = {
                    i + 1: Path(m["file_path"]) for i, m in enumerate(meetings_data)
                }
                ...
                for i, m in enumerate(meetings_data, 1):
                    ...
                    console.print(
                        f"  [bold green]{i}.[/bold green] {m['title']} ({datetime.datetime.fromisoformat(m['datetime_utc']).strftime('%Y-%m-%d %H:%M')})"
                    )
                ...
                try:
                    ...
                    if selected_meeting_file:
                        ...
                        console.print(
                            "[bold green]Meeting closed. Press any key to continue.[/bold green]"
                        )
                    else:
                        console.print(
                            f"[bold red]Invalid meeting number: {meeting_num}. Press any key to continue.[/bold red]"
                        )
                    _getch()
                except ValueError:
                    console.print(
                        "[bold red]Invalid input. Please enter a number. Press any key to continue.[/bold red]"
                    )
                    _getch()
                except Exception as e:
                    console.print(
                        f"[bold red]Error opening NVim: {e}. Press any key to continue.[/bold red]"
                    )
                    _getch()
            else:
                console.print(
                    "[bold yellow]No meetings found for this company. Press any key to continue.[/bold yellow]"
                )
                _getch()
        elif char == "c":
            while True:
                ...
                if contact_choice == "1":
                    ...
                    if not contact_name:
                        console.print(
                            "[bold yellow]Contact name cannot be empty. Press any key to continue.[/bold yellow]"
                        )
                        _getch()
                        continue

                    contact_email = typer.prompt(
                        "Enter contact email (optional)", default=""
                    )
                    contact_phone = typer.prompt(
                        "Enter contact phone (optional, e.g., +1-555-123-4567)",
                        default="",
                    )
                    contact_role = typer.prompt(
                        "Enter contact role (optional)", default=""
                    )
                    ...
                    new_person = Person(
                        ...
                        role=contact_role if contact_role else None,
                        slug=slugify(contact_name),
                    )  # Add slug here

                    people_dir = paths.people.path
                    person_dir = people_dir / slugify(
                        str(new_person.name) if new_person.name else new_person.slug
                    )
                    ...
                    if contact_symlink.exists():
                        console.print(
                            f"[bold yellow]Contact ''{new_person.name}'' already exists for this company. Press any key to continue.[/bold yellow]"
                        )
                    else:
                        os.symlink(
                            person_dir, contact_symlink, target_is_directory=True
                        )
                        console.print(
                            f"[bold green]New contact ''{new_person.name}'' added. Press any key to continue.[/bold green]"
                        )
                    _getch()
                    ...
                    if not people_dir.exists():
                        console.print(
                            "[bold red]People directory not found. Press any key to continue.[/bold red]"
                        )
                        _getch()
                        continue
                    ...
                    for person_dir_item in people_dir.iterdir():
                        if person_dir_item.is_dir():
                            ...
                            if person:
                                person_name = (
                                    str(person.name) if person.name else person.slug
                                )
                    ...
                    if not people_names:
                        console.print(
                            "[bold red]No people found in the people directory. Press any key to continue.[/bold red]"
                        )
                        _getch()
                        continue
                    ...
                    try:
                        ...
                        if selected_person_name:
                            ...
                            if contact_symlink.exists():
                                console.print(
                                    f"[bold yellow]Contact ''{selected_person_name}'' already exists for this company. Press any key to continue.[/bold yellow]"
                                )
                            else:
                                os.symlink(
                                    person_path,
                                ...
                                console.print(
                                    f"[bold green]Contact ''{selected_person_name}'' added. Press any key to continue.[/bold green]"
                                )
                        else:
                            console.print(
                                "[bold yellow]No contact selected. Press any key to continue.[/bold yellow]"
                            )

                    except subprocess.CalledProcessError:
                        console.print(
                            "[bold yellow]Contact selection cancelled. Press any key to continue.[/bold yellow]"
                        )
                    except Exception as e:
                        console.print(
                            f"[bold red]Error adding contact: {e}. Press any key to continue.[/bold red]"
                        )
                    _getch()
                    ...
                    if not contacts_dir.exists() or not any(contacts_dir.iterdir()):
                        console.print(
                            "[bold red]No contacts found for this company. Press any key to continue.[/bold red]"
                        )
                        _getch()
                        continue
                    ...
                    if not contact_names:
                        console.print(
                            "[bold red]No valid contacts found for this company. Press any key to continue.[/bold red]"
                        )
                        _getch()
                        continue
                    ...
                    try:
                        ...
                        if selected_person_name:
                            person_dir_to_edit = contact_person_dirs[
                            ...
                            if editor_command:
                                try:
                                    subprocess.run(
                                        [editor_command, str(person_index_path)],
                                        check=True,
                                    )
                                    console.print(
                                        "[bold green]Contact edited. Press any key to continue.[/bold green]"
                                    )
                                except Exception as e:
                                    console.print(
                                        f"[bold red]Error opening editor: {e}. Press any key to continue.[/bold red]"
                                    )
                            else:
                                console.print(
                                    "[bold red]No editor configured. Please configure an editor in your cocli_config.toml. Press any key to continue.[/bold red]"
                                )
                        else:
                            console.print(
                                "[bold yellow]No contact selected for editing. Press any key to continue.[/bold yellow]"
                            )

                    except subprocess.CalledProcessError:
                        console.print(
                            "[bold yellow]Contact editing cancelled. Press any key to continue.[/bold yellow]"
                        )
                    except Exception as e:
                        console.print(
                            f"[bold red]Error editing contact: {e}. Press any key to continue.[/bold red]"
                        )
                    _getch()
                    ...
                    try:
                        ...
                        if selected_person_name:
                            ...
                            if typer.confirm(
                                f"Are you sure you want to remove contact '{selected_person_name}' from this company?"
                            ):
                                symlink_to_delete.unlink()  # Remove the symlink
                                console.print(
                                    f"[bold green]Contact '{selected_person_name}' removed from company. Press any key to continue.[/bold green]"
                                )

                                if typer.confirm(
                                    f"Do you also want to permanently delete the person file for '{selected_person_name}' from the system?"
                                ):
                                    # Delete the person directory and its contents
                                    shutil.rmtree(person_dir_to_delete)
                                    console.print(
                                        f"[bold green]Person file for '{selected_person_name}' permanently deleted.[/bold green]"
                                    )
                                else:
                                    console.print(
                                        f"[bold yellow]Person file for '{selected_person_name}' retained.[/bold yellow]"
                                    )
                            else:
                                console.print(
                                    "[bold yellow]Contact removal cancelled. Press any key to continue.[/bold yellow]"
                                )
                        else:
                            console.print(
                                "[bold yellow]No contact selected for deletion. Press any key to continue.[/bold yellow]"
                            )

                    except subprocess.CalledProcessError:
                        console.print(
                            "[bold yellow]Contact deletion cancelled. Press any key to continue.[/bold yellow]"
                        )
                    except Exception as e:
                        console.print(
                            f"[bold red]Error deleting contact: {e}. Press any key to continue.[/bold red]"
                        )
                    _getch()
                    ...
                    console.print(
                        f"[bold red]Invalid option: '{contact_choice}'. Press any key to continue.[/bold red]"
                    )
                    _getch()
                ...
                try:
                    with tags_path.open("a+") as f:
                        ...
                        if new_tag not in tags:
                            f.write(f"{new_tag}\n")
                            console.print(
                                f"[bold green]Tag ''{new_tag}'' added. Press any key to continue.[/bold green]"
                            )
                        else:
                            console.print(
                                f"[bold yellow]Tag ''{new_tag}'' already exists. Press any key to continue.[/bold yellow]"
                            )
                except Exception as e:
                    console.print(
                        f"[bold red]Error adding tag: {e}. Press any key to continue.[/bold red]"
                    )
            else:
                console.print(
                    "[bold yellow]No tag entered. Press any key to continue.[/bold yellow]"
                )
            _getch()
        elif char == "T":
                    ...
                    try:
                        ...
                        if tag_to_remove in tags:
                            ...
                            with tags_path.open("w") as f:
                                f.write("\n".join(tags) + "\n")
                            console.print(
                                f"[bold green]Tag ''{tag_to_remove}'' removed. Press any key to continue.[/bold green]"
                            )
                        else:
                            console.print(
                                f"[bold yellow]Tag ''{tag_to_remove}'' not found. Press any key to continue.[/bold yellow]"
                            )
                    except Exception as e:
                        console.print(
                            f"[bold red]Error removing tag: {e}. Press any key to continue.[/bold red]"
                        )
                else:
                    console.print(
                        "[bold yellow]No tags file found. Press any key to continue.[/bold yellow]"
                    )
            else:
                console.print(
                    "[bold yellow]No tag entered. Press any key to continue.[/bold yellow]"
            ...
            if not potential_domains:
                console.print(
                    "No new potential domains found from contacts. Press any key to continue."
                )
                _getch()
                continue
            ...
            if len(potential_domains) == 1:
                if potential_domains[0] != company.domain:
                    if typer.confirm(
                        f"Found potential domain '{potential_domains[0]}'. Update company domain and enrich?"
                    ):
                        new_domain = potential_domains[0]
                else:
                    console.print(
                        f"Potential domain '{potential_domains[0]}' is already the company domain. Re-enriching. Press any key to continue."
                    )
                    new_domain = potential_domains[0]
                    _getch()
            else:
                ...
                choice_str = typer.prompt(
                    "Enter number of domain to set (or 's' to skip)"
                )
            ...
            if new_domain:
                ...
                console.print(
                    f"Company domain updated to '{new_domain}'. Triggering enrichment..."
                )
                ...
                try:
                    ...
                    console.print(
                        f"[bold red]HTTP request to enrichment service failed: {e}[/bold red]"
                    )
                except Exception as e:
                    console.print(
                        f"[bold red]Error processing enrichment response: {e}[/bold red]"
                    )

                if website_data:
                    company.last_enriched = datetime.datetime.now(
                        datetime.UTC
                    )  # Set last_enriched timestamp
                    console.print(
                        f"[bold green]Enrichment successful. New email: {website_data.email}[/bold green]"
                    )
                    ...
                    console.print(
                        "[bold green]Enrichment data saved and compiled. Press any key to continue.[/bold green]"
                    )
                else:
                    console.print(
                        "[bold yellow]Enrichment did not return data. Press any key to continue.[/bold yellow]"
                    )
            else:
            ...
            if not note_title:
                console.print(
                    "[bold yellow]Note title cannot be empty. Press any key to continue.[/bold yellow]"
                )
                _getch()
                continue

            console.print(
                "Enter note content (Press Ctrl+D to save, Ctrl+C to cancel):"
            )
            ...
            while True:
                try:
                    ...
                    console.print(
                        "[bold yellow]Note creation cancelled. Press any key to continue.[/bold yellow]"
                    )
                    _getch()
                    break
            ...
            if not note_content:
                console.print(
                    "[bold yellow]Note content cannot be empty. Press any key to continue.[/bold yellow]"
                )
                _getch()
                continue
            ...
            console.print(
                "[bold green]Note added. Press any key to continue.[/bold green]"
            )
            _getch()
        elif char == "N":
            ...
            if not notes_dir.exists() or not any(notes_dir.iterdir()):
                console.print(
                    "[bold yellow]No notes found to edit. Press any key to continue.[/bold yellow]"
                )
                _getch()
                continue
            ...
            if selected_note_filename:
                ...
                if editor_command:
                    try:
                        subprocess.run(
                            [editor_command, str(selected_note_path)], check=True
                        )
                        console.print(
                            "[bold green]Note edited. Press any key to continue.[/bold green]"
                        )
                    except Exception as e:
                        console.print(
                            f"[bold red]Error opening editor: {e}. Press any key to continue.[/bold red]"
                ...
                console.print(
                    "[bold yellow]No note selected for editing. Press any key to continue.[/bold yellow]"
                )
            _getch()
        elif char == "q":
            ...
            console.print(
                f"[bold red]Invalid option: '{char}'. Press 'a', 'e', 'w', 'p', 'm', 'C', or 'q'.[/bold red]"
            )
            _getch()  # Wait for a key press to clear the message
...
def view_meetings(
    company_name: str = typer.Argument(
        ..., help="Name of the company to view meetings for."
    ),
) -> None:
    """
...
def open_company_folder(
    company_name: str = typer.Argument(
        ..., help="Name of the company to open folder for."
    ),
) -> None:
    """
