...
async def pipeline(
    ...
    checkpoint_path = (
        paths.campaign(campaign_name).index(model_cls.INDEX_NAME).checkpoint
    )
    if prospect_type == "venue":
        checkpoint_path = (
            paths.campaign(campaign_name).index(model_cls.INDEX_NAME).path
            / "venues.checkpoint.usv"
        )
    ...
    with Progress(
        ...
        goal_task_id = progress.add_task(
            f"[cyan]Finding {prospect_type}s...", total=goal_limit
        )
        ...
        async with async_playwright() as p:
            ...
            async def consumer_task() -> None:
                ...
                while not stop_event.is_set():
                    ...
                    for msg in messages:
                        ...
                        progress.update(
                            scan_task_id,
                            description=f"[dim]Enriching: {msg.domain}[/dim]",
                        )
            ...
            async def producer_task(existing_companies_map: Dict[str, str]) -> None:
                ...
                async for list_item in prospect_generator:
                    ...
                    progress.update(
                        scan_task_id,
                        description=f"[bold blue]Detailing:[/bold blue] {list_item.name}",
                    )
                    ...
                    try:
                        ...
                        if detailed_data_raw:
                            ...
                            from cocli.models.campaigns.indexes.google_maps_raw import (
                                GoogleMapsRawResult,
                            )
                            ...
                            if prospect_type == "venue" and (
                                not resource_discovery or is_val
                            ):
                                # Cast to Any to satisfy mypy's strict class-method checking on Union types
                                cast(Any, model_cls).append_to_checkpoint(
                                    campaign_name, detailed_data
                                )
                                console.print(
                                    f"[bold green]VENUE CAPTURED:[/bold green] {detailed_data.name}"
                                )
                                results_found_count += 1
                                progress.advance(goal_task_id)
                                if results_found_count >= goal_limit:
                            ...
                            if detailed_data.domain:
                                enrichment_queue.push(
                                    QueueMessage(
                                        ...
                                        company_slug=detailed_data.company_slug,
                                        campaign_name=campaign_name,
                                        force_refresh=force,
                                        ack_token=None,
                                    )
            ...
            try:
                async with asyncio.timeout(1800):  # 30-minute watchdog
                    await asyncio.gather(
                        producer_task(existing_companies_map), consumer_task()
                    )
...
def queue_scrapes(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Report task count without enqueuing."
    ),
) -> None:
    ...
    if grid_tiles:
        ...
        for tile in grid_tiles:
            ...
            if lat and lon and tile_id:
                ...
                bounds = {
                    "lat_min": float(lat) - 0.05,
                    "lat_max": float(lat) + 0.05,
                    "lon_min": float(lon) - 0.05,
                    "lon_max": float(lon) + 0.05,
                }
                for phrase in search_phrases:
                    if not force:
                        # 1. Direct Tile Check (Fastest)
                        match_area = scrape_index.is_tile_scraped(
                            phrase, tile_id, ttl_days=30
                        )
                        if match_area:
                            continue

                        # 2. Overlap Check (Legacy and Robustness)
                        match = scrape_index.is_area_scraped(
                            phrase, bounds, overlap_threshold_percent=90.0
                        )
    ...
    if dry_run:
        ...
        covered_pct = (
            (covered / total_combinations * 100) if total_combinations > 0 else 0
        )
        ...
        console.print(
            f"  Already Covered:                {covered} ({covered_pct:.1f}%)"
        )
        console.print(
            f"  [bold yellow]Remaining to Scrape:            {len(tasks_to_queue)}[/bold yellow]"
        )
        return
    ...
    if tasks_to_queue:
        ...
        queue_manager = get_queue_manager(
            "scrape_tasks", use_cloud=True, queue_type="scrape"
        )
        ...
        with Progress(
            ...
            task_id = progress.add_task(
                f"[cyan]Pushing {len(tasks_to_queue)} tasks...[/cyan]",
                total=len(tasks_to_queue),
            )
        ...
        console.print(
            f"[bold green]Successfully queued {len(tasks_to_queue)} scrape tasks.[/bold green]"
        )
    else:
        console.print(
            "[yellow]No tasks to queue (all areas covered or no targets found).[/yellow]"
        )
...
def queue_batch(
    ...
    force: bool = typer.Option(False, "--force"),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Show tasks without enqueuing."
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Save tasks to a JSON file instead of enqueuing."
    ),
    no_limit: bool = typer.Option(
        False, "--no-limit", help="Ignore the limit and process all unscraped areas."
    ),
) -> None:
    """
                ...
                for phrase in search_phrases:
                    if not force:
                        if scrape_index.is_tile_scraped(phrase, tile_id, ttl_days=30):
                            continue
                        if scrape_index.is_area_scraped(
                            phrase, bounds, overlap_threshold_percent=90.0
                        ):
                            continue
    ...
    if tasks_to_queue:
        if output:
            ...
            console.print(
                f"[bold green]Successfully exported {len(tasks_to_queue)} tasks to {output}[/bold green]"
            )
            return

        if dry_run:
            console.print(
                f"[bold blue]Dry Run: Would have queued {len(tasks_to_queue)} tasks:[/bold blue]"
            )
            for t_dict in tasks_to_queue:
                console.print(
                    f" - {t_dict['search_phrase']} @ {t_dict['latitude']}, {t_dict['longitude']} (Tile: {t_dict['tile_id']})"
                )
            return
        ...
        console.print(
            f"[bold green]Successfully queued {len(tasks_to_queue)} tasks.[/bold green]"
        )
    else:
...
def prepare_mission(
    ...
    ttl_days: int = typer.Option(
        30,
        "--ttl-days",
        help="Tiles scraped longer than this many days ago are considered stale and added to the frontier.",
    ),
    debug: bool = typer.Option(
        False,
        "--debug",
        help="Enable diagnostic logging and generate scraped manifest.",
    ),
) -> None:
    """
...
def create_batch(
    ...
    offset: Optional[int] = typer.Option(
        None, help="Manual override for the frontier offset"
    ),
    query: Optional[str] = typer.Option(
        None, help="Manual task entry (format: 'tile_id;phrase;lat;lon')"
    ),
) -> None:
    """
    ...
    if query:
        ...
        if len(parts) == 4:
            ...
            console.print(
                "[red]Invalid query format. Expected 'tile_id;phrase;lat;lon'[/red]"
            )
            raise typer.Exit(1)
    else:
        # Frontier Mode
        if not frontier_path.exists():
            console.print(
                f"[red]Frontier file not found: {frontier_path}. Run 'prepare-mission' first.[/red]"
            )
            raise typer.Exit(1)
            ...
            for i, line in enumerate(f):
                ...
                if line.strip():
                    try:
                        ...
                        match = scrape_index.is_tile_scraped(
                            task.search_phrase, task.tile_id, ttl_days=30
                        )
                        if not match:
                            tasks.append(task)
                        else:
...
def build_mission_index(
    ...
    batch: Optional[str] = typer.Option(
        None, help="Specific batch name to activate (from pending/batches/)"
    ),
) -> None:
    """
...
def monitor_batch(
    ...
    recent: bool = typer.Option(
        False, "--recent", help="Only show recent or pending tasks."
    ),
    cluster: bool = typer.Option(
        False,
        "--cluster",
        help="Run report remotely on the cluster Hub for real-time data.",
    ),
) -> None:
    """
...
def discover_venues(
    ...
    goal_limit: int = typer.Option(
        20, "--limit", "-l", help="Number of venues to find."
    ),
    force: bool = typer.Option(False, "--force", "-f"),
    headed: bool = typer.Option(
        False, "--headed", help="Run with a visible browser window."
    ),
) -> None:
    """
    ...
    checkpoint_path = (
        paths.campaign(campaign_name).index(GoogleMapsProspect.INDEX_NAME).path
        / "venues.checkpoint.usv"
    )
    console.print(
        f"[bold blue]Starting Venue Discovery for {campaign_name}...[/bold blue]"
    )
    ...
    asyncio.run(
        pipeline(
            ...
            target_locations=[{"city": loc} for loc in locations]
            if locations
            else None,
            prospect_type="venue",
        )
...
def achieve_goal(
    goal_limit: int = typer.Option(
        ...
        "--emails",
        "--limit",
        help="Number of resources (or emails) to find before stopping.",
    ),
    campaign_name: Optional[str] = typer.Argument(None),
    force: bool = typer.Option(False, "--force", "-f"),
    ...
    resource_discovery: bool = typer.Option(
        False,
        "--resource-discovery",
    ...
    if proximity_miles == 10.0 and "proximity" in prospecting_config:
        ...
        console.print(
            f"[dim]Using proximity from campaign config: {proximity_miles} mi[/dim]"
        )
