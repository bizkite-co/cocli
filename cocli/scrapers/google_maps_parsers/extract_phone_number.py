...
def extract_phone_number(soup: BeautifulSoup, inner_text: str, debug: bool = False) -> str:
    ...
    if phone_match:
        ...
        if debug: logger.debug(f"Extracted Phone_1 (innerText): {phone_number}")
    else:
        ...
        if phone_element:
            ...
            if debug: logger.debug(f"Extracted Phone_1 (HTML fallback): {phone_number}")
        else:
            if debug: logger.debug("Phone number element not found from innerText or HTML fallback.")
    return phone_number