...
def extract_gmb_url_coordinates_place_id(soup: BeautifulSoup, debug: bool = False) -> Dict[str, str]:
    ...
    if gmb_url_element and gmb_url_element.has_attr("href"):
        ...
        if debug:
            logger.debug(f"Extracted GMB_URL (HTML): {gmb_url}")
        ...
        if match:
            ...
            if debug:
                logger.debug(f"Extracted Coordinates (HTML): {coordinates}")
        else:
            if debug:
                logger.debug("Coordinates not found in GMB_URL.")

        place_id_match = re.search(r'(ChI[a-zA-Z0-9_-]+)', gmb_url)
        if place_id_match:
            place_id = place_id_match.group(1)
            if debug:
                logger.debug(f"Extracted Place_ID (HTML): {place_id}")
        else:
            if debug:
                logger.debug("Place_ID not found in GMB_URL.")
    else:
        if debug:
            logger.debug("GMB URL element not found (HTML).")
