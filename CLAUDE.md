...
**TUI double pane/table headers (Windows Terminal + Sixel):** the recurring "double titles on two rows" bug is NOT a widget-structure or `border_title` issue - three border_title fixes (57690e62, 5cb130c1, 7eada11e) could never work because no code sets non-empty titles and Textual 6 renders border titles from the private `_border_title`, which stays `None`. Actual cause (2026-09-13): `textual_image`'s Sixel widget injects raw DCS + absolute-cursor escapes into Textual's *differential* updates; Windows Terminal's sixel cursor/scroll handling desyncs the compositor, leaving stale rows. Fix (2026-09-14, `cocli/utils/textual_utils.py:get_image_widget_class`): (1) the sixel payload is clamped half a cell short of the widget's bottom edge so WT's sub-cell rasterization overflow can't scroll the viewport and duplicate the bottom row (the stale-keymaps-above-Footer glitch); (2) a flag-gated two-stage heal runs after every sixel-injecting frame - a full absolute-positioned repaint, then a text-only pass over the rows below the image that contains no DCS and therefore cannot re-create the corruption it erases. Escape hatches: `COCLI_IMAGE_BACKEND=halfcell|unicode|tgp|none`; `scripts/wt_sixel_repro.py` is a dependency-free repro for a microsoft/terminal upstream issue. WezTerm: deferred 2026-09-14 - textual_image's flaky DA1 probe fails there (falls back to pixelated halfcell); force with `COCLI_IMAGE_BACKEND=sixel` and enable `enable_sixel_windows` in wezterm.lua if revisiting. Regression tests in `tests/tui/test_screenshot_image_backend.py` assert on *rendered output* (single header rows), not suppression mechanics.
