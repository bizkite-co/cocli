...
def extract_gmb_url_coordinates_place_id(soup: BeautifulSoup, debug: bool = False) -> Dict[str, str]:
    ...
    if gmb_url_element and gmb_url_element.has_attr("href"):
        ...
        if debug: print(f"Debug: Extracted GMB_URL (HTML): {gmb_url}")
        ...
        if match:
            ...
            if debug: print(f"Debug: Extracted Coordinates (HTML): {coordinates}")
        else:
        ...
        if place_id_match:
            ...
            if debug: print("Debug: Place_ID not found in GMB_URL.")
    else:
        if debug: print("Debug: GMB URL element not found (HTML).")
