...
def identify_candidates() -> None:
    ...
    for company in Company.get_all():
        ...
        # 2. Match Niche
        cat_match = any(
            pc in [c.lower() for c in company.categories] for pc in prime_categories
        )
        kw_match = (
            must_have_keyword in [k.lower() for k in company.keywords]
            or must_have_keyword in (company.description or "").lower()
        )
