...
def populate_to_call(
    campaign_name: str, count: int = 40, skip_email_check: bool = False
) -> None:
    ...
    if not skip_email_check:
        ...
        print("Skipping email domain check...")
    ...
    for usv_file in files_to_process:
        with open(usv_file, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                website = row[19]
                if not website:
                    # If we aren't checking for emails, we still need a website for the task
                    if not skip_email_check:
                        continue

                domain = get_base_domain(website) if website else ""

                # Check email match if not skipped
                if not skip_email_check and (
                    not domain or domain not in enriched_domains
                ):
                    continue
                ...
                try:
                    reviews_count = (
                        int("".join(filter(str.isdigit, reviews_count_str)))
                        if reviews_count_str
                        else 0
                    )
                    average_rating = (
                        float(average_rating_str.strip('"'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("campaign", nargs="?", default="roadmap")
    parser.add_argument("--count", type=int, default=40)
    parser.add_argument("--skip-email-check", action="store_true")
    args = parser.parse_args()

    populate_to_call(args.campaign, args.count, args.skip_email_check)
